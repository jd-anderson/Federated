...
def get_dataset(args):
    ...
    if args.dataset == 'cifar':
        ...
        os.chdir('..')
        data_set_name = 'synthetic_' + str(args.syn_alpha_beta) + '_' + str(args.syn_alpha_beta)
        train_path = os.path.join('data', data_set_name, 'data', 'train')
        test_path = os.path.join('data', data_set_name, 'data', 'test')
        ...
        for user in clients:
            train_data_new.extend([[np.array(x, dtype=np.float), np.array(y, dtype=np.int64)] for x, y in
                                   zip(train_data[user]['x'], train_data[user]['y'])])
            test_data_new.extend([[np.array(x, dtype=np.float), np.array(y, dtype=np.int64)] for x, y in
                                  zip(test_data[user]['x'], test_data[user]['y'])])

        idxs_train = np.arange(sum(len(train_data[key]['x']) for i, key in enumerate(train_data.keys())))
        train_dataset = DatasetSplit(train_data_new, idxs_train)
        idxs_test = np.arange(sum(len(test_data[key]['x']) for i, key in enumerate(test_data.keys())))
        test_dataset = DatasetSplit(test_data_new, idxs_test)
...
def read_data(train_data_dir, test_data_dir, num_users):
    ...
    for user in train_clients:
        ...
        dict_users[i % num_users] = np.concatenate(
            (dict_users[i % num_users], np.arange(train_size, train_size + length_train)), axis=0)

        train_data_new.extend(
            [[np.array(x, dtype=np.float).reshape([1, 28, 28]), np.array(y, dtype=np.int64)] for x, y in
             zip(train_data[user]['x'], train_data[user]['y'])])
