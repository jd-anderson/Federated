...
class FedDR_SGD(Optimizer):
    def __init__(self, params, lr=required, mu=1.0, freq=10):
        ...
        self.counter_in = 0
        self.flag = False
        super(FedDR_SGD, self).__init__(params, defaults)
    def __setstate__(self, state):
        super(FedDR_SGD, self).__setstate__(state)
    def step(self, closure=None):
        """Performs a single optimization step.
        Arguments:
            closure (callable, optional): A closure that reevaluates the model
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if (self.counter_in == 0):
                    if not (self.flag):  # first iteration, initialize
                        x_0.copy_(p.data)
                        lamb.copy_(p.data)# x_0 = x_i
                    else:  # after the first iteration
                        lamb.add_(1, p.data - x_0)
                        # temp = p.data.clone().detach()
                        p.data.copy_(x_0)
                        # x_0.copy_(temp)
                if (self.counter_in > 0):
                    d_p = p.grad.data# first inner loop, only switch x_0 and x_i
                    p.data.add_(-group['lr'], d_p+(p.data - lamb)*mu)
                    # p.data.add_(-group['lr'], d_p)
                if (self.counter_in + 1 == freq):  # last inner loop, perform update on lambda and x_0
                    x_0.copy_(p.data)
                    p.data.add_(1,p.data-lamb)
        self.flag = True
        self.counter_in += 1
        if self.counter_in == freq:
            self.counter_in = 0
