...
from optimizer import PSVRG, FedPD_SGD, FedPD_VR, PSGD, FedDR_SGD
...
class Agent:
    def __init__(self, model, args, agent_id, criterion):
        ...
        elif args.optimizer == 'FedPD' or args.optimizer == 'FedADMM':
            if args.VR:
        ...
        elif args.optimizer == 'FedDR':
          	self.optimizer = FedDR_SGD(self.local_model.parameters(), lr=args.lr, mu=args.mu, freq=args.freq_in)
        self.id = agent_id
        self.VR = args.VR
    ...
    def train_(self, model_dict, num_its, dataset, idxs, update_model, compute_full):
        ...
        if self.opti == 'FedPD' or self.opti == 'FedDR' or self.opti == 'FedADMM':
